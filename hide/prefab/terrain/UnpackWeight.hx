...
class UnpackWeight extends h3d.shader.ScreenShader {
	...
	static var SRC = {
		...
		function fragment() {
			...
			if(i2 == index) pixelColor = vec4(vec3(packedWeightTexture.get(calculatedUV).g), 1.0);
			if(i3 == index) pixelColor = vec4(vec3(packedWeightTexture.get(calculatedUV).b), 1.0);
		}
