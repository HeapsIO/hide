...
class Prefab extends hide.view.FileView {
	...
	override function onDisplay() {
		...
		data = hrt.prefab2.Prefab.createFromDynamic(haxe.Json.parse(content));
	...
	function applyGraphicsFilter(typeid: String, enable: Bool)
	{
		...
		var all = data.getAll(hrt.prefab2.Object3D, true);
		for (obj in all) {
	...
	function applySceneFilter(typeid: String, visible: Bool) {
		...
		var all = [];
		if (typeid != 'light')
			all = data.getAll(hrt.prefab2.Prefab, true);
		else
			all = data.flatten(hrt.prefab2.Prefab);
		for(p in all) {
	...
	function applySceneStyle(p: PrefabElement) {
		var prefabView = Std.downcast(p, hrt.prefab2.Prefab); // don't use "to" (Reference)
		...
		if(color != null){
			...
			var box = p.to(hrt.prefab2.l3d.Box);
			if(box != null) {
				box.setColor(color);
			}
			var poly = p.to(hrt.prefab2.l3d.Polygon);
			if(poly != null) {
				poly.setColor(color);
			}
		}
