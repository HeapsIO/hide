...
class LightProbeBaker {
	...
	public function initScene( sceneData : hrt.prefab2.Prefab, scene : hide.comp2.Scene , env : h3d.scene.pbr.Environment) {
		if(offScreenScene != null) offScreenScene.dispose();
		...
		function filter( p : hrt.prefab2.Prefab ) {
			for( c in p.children ) {
				if(!keep(c))
	...
	public function bake( volumetricLightMap : hrt.prefab2.vlm.VolumetricMesh, resolution : Int, ?time :Float ) {
	...
	function convertOuputTexturesIntoSH( volumetricLightMap : hrt.prefab2.vlm.VolumetricMesh, pixelsOut : hxd.Pixels ) {

		var order = volumetricLightMap.shOrder;
		var sh = new hrt.prefab2.vlm.SphericalHarmonic(order);
