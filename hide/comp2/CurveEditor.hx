...
typedef CurveKey = hrt.prefab2.Curve.CurveKey;
...
class CurveEditor extends hide.comp.Component {
	...
	public var curve(default, set) : hrt.prefab2.Curve;
	...
	function set_curve(curve: hrt.prefab2.Curve) {
		this.curve = curve;
	...
	function fixKey(key : CurveKey) {
		...
		inline function addPrevH() {
			if(key.prevHandle == null)
				key.prevHandle = new hrt.prefab2.Curve.CurveHandle(prev != null ? (prev.time - key.time) / 3 : -0.5, 0);
		}
		inline function addNextH() {
			if(key.nextHandle == null)
				key.nextHandle = new hrt.prefab2.Curve.CurveHandle(next != null ? (next.time - key.time) / 3 : -0.5, 0);
		}
		switch(key.mode) {
			case Aligned:
				addPrevH();
	...
	public function refreshGraph(?anim: Bool = false, ?animKey: CurveKey) {
		...
		function editPopup(key: CurveKey, top: Float, left: Float) {
			...
			function setMode(m: hrt.prefab2.Curve.CurveKeyMode) {
				key.mode = m;
				curve.keyMode = m;
				fixKey(key);
				refreshGraph();
