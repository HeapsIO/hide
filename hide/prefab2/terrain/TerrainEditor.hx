...
using Lambda;
import hxd.Pixels;
import hrt.prefab2.terrain.Tile;
import hxd.Key as K;
...
class TerrainRevertData {
	...
	public var surface : hrt.prefab2.terrain.Surface;
	public function new(){
...
@:access(hrt.prefab2.terrain.TerrainMesh)
@:access(hrt.prefab2.terrain.Terrain)
@:access(hrt.prefab2.terrain.Tile)
class TerrainEditor {

	public var currentBrush = new Brush();
	public var currentSurface : hrt.prefab2.terrain.Surface;
	...
	// Edition
	var brushPreview : hide.prefab2.terrain.Brush.BrushPreview;
	var interactive : h2d.Interactive;
	var remainingDist = 0.0;
	var lastPos : h3d.col.Point;
	var lastMousePos : h2d.col.Point;
	var lastBrushSize : Float;
	var heightStrokeBufferArray : hide.prefab2.terrain.StrokeBuffer.StrokeBufferArray;
	var weightStrokeBufferArray : hide.prefab2.terrain.StrokeBuffer.StrokeBufferArray;
	// Shader for edition
	var copyPass = new h3d.pass.Copy();
	var normalizeWeight = new h3d.pass.ScreenFx(new hide.prefab2.terrain.NormalizeWeight());
	var clampWeight = new h3d.pass.ScreenFx(new hide.prefab2.terrain.ClampWeight());
	var generateIndex = new h3d.pass.ScreenFx(new hide.prefab2.terrain.GenerateIndex());
	var swapIndex = new h3d.pass.ScreenFx(new hide.prefab2.terrain.SwapIndex());
	var setHeight = new h3d.pass.ScreenFx(new hide.prefab2.terrain.SetHeight());
	var smoothHeight = new h3d.pass.ScreenFx(new hide.prefab2.terrain.SmoothHeight());
	// Revert
	var terrainPrefab : hrt.prefab2.terrain.Terrain;
	...
	var customRenderer = new hide.prefab2.terrain.CustomRenderer("terrainUV");

	public function new( terrainPrefab : hrt.prefab2.terrain.Terrain, undo : hide.ui.UndoHistory ) {
		...
		brushPreview = new hide.prefab2.terrain.Brush.BrushPreview(terrainPrefab.terrain);

		heightStrokeBufferArray = new hide.prefab2.terrain.StrokeBuffer.StrokeBufferArray(R32F,new h2d.col.IPoint(terrainPrefab.terrain.heightMapResolution.x, terrainPrefab.terrain.heightMapResolution.y));
		weightStrokeBufferArray = new hide.prefab2.terrain.StrokeBuffer.StrokeBufferArray(R8, terrainPrefab.terrain.weightMapResolution);
	...
	function renderTerrainUV() {
		...
		for( i in 0 ... tiles.length ) {
			...
			var s = new hide.prefab2.terrain.CustomUV();
