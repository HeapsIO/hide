...
enum PrefabFieldType {
	PInt;
	PFloat;
	PBool;
	PString;
	PEnum( name : String );
	PArray( k : PrefabFieldType );
	PUnknown;
	PDynamic;
}
...
class Macros {
	...
	public static macro function getSerializableProps() : Expr {
		...
		var serArrayExprs = [
			for (f in serializableProps) {
				...
				var type = getPrefabFieldType(f.type);
				props.push({
					field: "type",
					expr: macro $v{type},
				});
	...
	static function getPrefabFieldType(t : haxe.macro.Type) {
		var type = switch( t ) {
			case TAbstract(a, pl):
				switch( a.toString() ) {
