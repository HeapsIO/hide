...
class HuiTree<TreeItem> extends HuiElement {
	...
	**/
	...
	/**
		Open `item` and recursively all of its descendants.
		If `item` is null, the whole tree is opened.
	**/
	public function openAll(?item: TreeItem) : Void {
		if (refreshFlags.toInt() != 0) {
			afterRefreshCallbacks.push(openAll.bind(item));
			return;
		}

		function rec(data: TreeItemData) {
			toggleItemDataOpen(data, true);
			// children are generated by toggleItemDataOpen when the item gets opened
		...
		forEachRoot(item, rec);
	}

	/**
		Close `item` and recursively all of its descendants.
		If `item` is null, the whole tree is closed.
	**/
	public function foldAll(?item: TreeItem) : Void {
		if (refreshFlags.toInt() != 0) {
			afterRefreshCallbacks.push(foldAll.bind(item));
			return;
		}

		function rec(data: TreeItemData) {
			// only walk the already known children, the other ones are closed anyway
			if (data.children != null) {
				for (child in data.children)
					rec(child);
			}
	...
	/**
		Call `f` on `item`, or on each root of the tree if `item` is null
	**/
	function forEachRoot(item: Null<TreeItem>, f: (data: TreeItemData) -> Void) : Void {
		if (item == null) {
	...
	**/
	...
	function openContextMenu(item: TreeItem) : Void {
		...
		var hasItem = item != null && hasChildren(item);
		entries.push({label: "Fold All", click: () -> foldAll(null)});
		entries.push({label: "Open All", click: () -> openAll(null)});
		entries.push({label: "Fold Children", enabled: hasItem, click: () -> foldAll(item)});
		entries.push({label: "Open Children", enabled: hasItem, click: () -> openAll(item)});
